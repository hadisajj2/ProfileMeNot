...
PROFILEMENOT.editor = function () {
      		...
      		var wm = Components.classes["@mozilla.org/appshell/window-mediator;1"].
        		...
        		var en = wm.getEnumerator("navigator:browser");
        		 while (en.hasMoreElements()) { 
        	      var win = en.getNext(); 
        	      if (win.PROFILEMENOT && win.PROFILEMENOT.gTrackMeNot) 
        	      	return win;
        	    }
    	    return null		
  }
  ...
  function cout(msg) {
        var win = getRunningWindow();
      	win.PROFILEMENOT.utils.cout(msg);
  	}
  ...
  function	resetTree (id,_window) {
    ...
    function getRSSFile() {
        ...
        rss_file = PROFILEMENOT.utils.getProfileDir().clone();
	      rss_file.append("PROFILEMENOT");
	...
	return { 
	    _loadTree : function(reload) {  
	        if (!rss_obj || reload) PROFILEMENOT.editor._loadRSS();
    ...
    _deleteItem: function () {
      ...
      var catIndex =   father.children.indexOf(cat);
  		...
  		PROFILEMENOT.editor._saveRSS();
  		PROFILEMENOT.editor._loadTree();
    },
    ...
    _getObjByType: function( obj, type, result) {
      for each(var elt in obj.children ) {
          ...
          PROFILEMENOT.editor._getObjByType( elt, type, result);
      }
      return result;    
    },
    
    _getRSSObj: function () {
        if (!rss_obj) PROFILEMENOT.editor._loadRSS();
        return rss_obj;   
    },
    ...
    _loadEntryWindow : function(id) {
        ...
        rss_obj = window.opener.PROFILEMENOT.editor._getRSSObj(); 
    },
    ...
    _saveEntryChange: function(old_id) {
        ...
        window.opener.PROFILEMENOT.editor._setRSSObj(rss_obj)
        window.opener.PROFILEMENOT.editor._loadTree();
        return;
    },
    ...
    _updateTextLabel : function () {
      ...
      return;	
		...
		_onSelectTopicLoad: function(e) {
			PROFILEMENOT.editor._loadCategoryTree();
    ...
    _saveRSS : function () {
      var file = getRSSFile();
  		var foStream = PROFILEMENOT.utils.getFoStream();
